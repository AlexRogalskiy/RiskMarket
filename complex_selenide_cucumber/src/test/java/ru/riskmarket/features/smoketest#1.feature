Feature: smoke test #1, go through the service to button "Купить"
  ...
  Scenario: go through the service to button "Купить"
    ...
    And press button with text "Вход в кабинет" on "first page"
    And type to input with name "Логин" text: "riskmarket.testoviy2016@yandex.ru" on "first page"
    And type to input with name "Пароль" text: "l0dcfJMB" on "first page"
    And press button with text "Войти" on "first page"
    ...
    When press button with text "Рассчитать полис" on "first page"
  ...
  Scenario: go through service to yandex pay-page
    ...
    Given press button with text "КУПИТЬ" on "second page"
    #actions at third page
    And type to input with name "lastName" text: "TESTOVIY" on "third page"
    And type to input with name "firstName" text: "TEST" on "third page"
    And press button with text "Принимаю" on "third page"
